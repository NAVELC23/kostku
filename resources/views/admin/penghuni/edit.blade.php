...
<div class="container mx-auto px-4 py-6">
    <div class="flex justify-between items-center mb-6">
        <h1 class="text-2xl font-bold">Edit Penghuni</h1>
    ...
    <div class="bg-white rounded shadow p-6 max-w-xl">
        <form action="{{ route('admin.penghuni.update', $penghuni->id_penghuni) }}" method="POST">
            @csrf
            @method('PUT')
                ...
                <select name="id_user" class="w-full border rounded px-3 py-2">
                    ...
                    @foreach($users as $user)
                        <option value="{{ $user->id }}"
                            {{ old('id_user', $penghuni->id_user) == $user->id ? 'selected' : '' }}>
                ...
                <select name="id_kamar" class="w-full border rounded px-3 py-2">
                    ...
                    @foreach($kamars as $kamar)
                        <option value="{{ $kamar->id_kamar }}"
                            {{ old('id_kamar', $penghuni->id_kamar) == $kamar->id_kamar ? 'selected' : '' }}>
            ...
            <div class="mb-4">
                ...
                <select name="status_penghuni" class="w-full border rounded px-3 py-2">
                    <option value="aktif" {{ old('status_penghuni', $penghuni->status_penghuni) == 'aktif' ? 'selected' : '' }}>Aktif</option>
                    <option value="nonaktif" {{ old('status_penghuni', $penghuni->status_penghuni) == 'nonaktif' ? 'selected' : '' }}>Nonaktif</option>
                </select>
