...
<div class="max-w-6xl mx-auto px-6 py-12">
    ...
    {{-- Grid kamar --}}
    <div id="kamar-grid" class="grid grid-cols-1 md:grid-cols-3 gap-6 hidden"></div>

    {{-- Kosong --}}
    <div id="kamar-empty" class="hidden col-span-3 text-center text-gray-500 py-12">
...
fetch('/api/kamar')
    ...
    .then(response => {
        ...
        const empty = document.getElementById('kamar-empty');
        ...
        kamars.forEach(kamar => {
            const statusClass = kamar.status === 'Tersedia'
                ? 'bg-green-100 text-green-700'
                : 'bg-red-100 text-red-700';

            const foto = kamar.foto
                ? `<img src="/storage/${kamar.foto}" alt="Kamar ${kamar.nomor_kamar}" class="w-full h-48 object-cover">`
                : `<div class="w-full h-48 bg-emerald-100 flex items-center justify-center text-emerald-400 text-5xl">🛏️</div>`;

            const fasilitas = kamar.fasilitas && kamar.fasilitas.length > 0
                ? kamar.fasilitas.map(f => f.nama_fasilitas).join(', ')
                : '-';

            const harga = new Intl.NumberFormat('id-ID').format(kamar.harga);
            ...
            grid.innerHTML += `
                <div class="bg-white rounded-lg shadow overflow-hidden">
                    ...
                    <div class="p-5">
                        ...
                        <p class="text-sm text-gray-500 mb-3">Fasilitas: ${fasilitas}</p>
                        <p class="text-emerald-700 font-bold text-lg">
                            Rp ${harga}<span class="text-sm font-normal text-gray-500">/bulan</span>
                        </p>
                    </div>
                </div>
            `;
    ...
    .catch(err => {
        document.getElementById('loading').textContent = 'Gagal memuat data kamar.';
        console.error(err);
    });
</script>
